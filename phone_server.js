...
var selectorFromUserQuery = function(user) {
  if (user.id) return {_id: user.id};
  else if (user.phone) return {"phone.number": user.phone};
...
Accounts.registerLoginHandler("phone", function(options) {
  ...
  if (
    !user.services ||
    !user.services.phone ||
    !(user.services.phone.bcrypt || user.services.phone.srp)
  )
...
Accounts.sendPhoneVerificationCode = function(userId, phone) {
  ...
  if (verifyObject.numOfRetries > maxRetryCounts) {
    ...
    var waitTimeBetweenMaxRetries =
      Accounts._options.verificationRetriesWaitTime;
    nextRetryDate = new Date(
      verifyObject.lastRetry.getTime() + waitTimeBetweenMaxRetries
    );
    if (nextRetryDate > curTime) {
      var waitTimeInMin = Math.ceil(
          Math.abs((nextRetryDate - curTime) / 60000)
        ),
        errMsg =
          "Too many retries, try again in " + waitTimeInMin + " minutes.";
      throw new Error(errMsg);
    }
  }
...
var createUser = function(options) {
  ...
  try {
    ...
    if (match[1].indexOf("users.$phone.number") !== -1)
      throw new Meteor.Error(
        403,
        "Phone number already exists, failed on creation."
      );
    throw e;
  }
};
...
var normalizePhone = function(phone) {
  ...
  if (
    ...
    Accounts._options.adminPhoneNumbers.indexOf(phone) != -1
  ) {
    return phone;
  }
